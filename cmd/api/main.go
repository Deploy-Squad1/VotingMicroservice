...
func main() {
	...
	mux.HandleFunc("GET /voting/api/polls", middlewares.RequireRole()(pollHandler.GetAllActivePolls))
	mux.HandleFunc("POST /voting/api/polls/upgrade", middlewares.RequireRole()(pollHandler.CreateUpgradePoll))
	mux.HandleFunc("POST /voting/api/polls/kick/user/{target_id}", middlewares.RequireRole()(pollHandler.CreateKickPoll))
	mux.HandleFunc("POST /voting/api/polls/{poll_id}/vote", middlewares.RequireRole("Silver", "Gold")(pollHandler.VoteOnPoll))

	// Internal endpoints
	mux.HandleFunc("POST /voting/api/internal/polls/process", pollHandler.ProcessExpiredPolls)
	mux.HandleFunc("DELETE /voting/api/internal/database/delete", databaseMigrationHandler.ApplyMigration)
